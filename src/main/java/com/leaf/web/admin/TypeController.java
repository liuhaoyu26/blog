...
public class TypeController {
	...
	@GetMapping("/types")
	public String type(@PageableDefault(size = 4,sort = {"id"},direction = Sort.Direction.DESC) Pageable pageable, Model model){
	...
	public String saveType(@Valid Type type, BindingResult result,RedirectAttributes attributes){
		...
		if(t == null){
			...
			attributes.addFlashAttribute("message","恭喜你,新增成功!!!");
		}
	...
	public String editType(@Valid Type type, BindingResult result,@PathVariable Long id,RedirectAttributes attributes){
		...
		if(t == null){
			...
			attributes.addFlashAttribute("message","恭喜你,更新成功!!!");
		}
		return "redirect:/admin/types";
	}
